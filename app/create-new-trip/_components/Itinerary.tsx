...
function Itinerary() {
  ...
  const [trip_data , setTripData] = useState<TripInfo | null>(null);
  ...
  const data = [
    {
      ...
      content: (
        <div className="flex flex-col gap-4">
          {(trip_data?.hotels ?? []).map((hotel: Hotel, idx: number) => (
            <HotelCard hotel={hotel} key={idx} />
          ))}
        </div>
      ),
    },
    ...(trip_data?.itinerary ?? []).map((dayData, idx: number) => ({
      title: `Day ${dayData?.day ?? ''}`, // Fallback to empty string if day is missing
      content: (
        <div className="flex flex-col gap-3">
          <p>
            Best Time: <span className="text-primary font-bold">{dayData?.best_time_to_visit_day ?? 'N/A'}</span>
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {(dayData?.activities ?? []).map((item: Activity, idx: number) => (
